import Box, { withBox } from "@ps/ui/components/content/atoms/Box";
import { withButton } from "@ps/ui/components/form/atoms/Button";
...
export const MediaQueriesDemo = () => (
  <>
    <Box as="h5" ss="margin-top:1rem;">
      These <span className="gold">props </span> apply to your current <span className="orange">page size</span>:
    </Box>
    <pre>{`<Box `}</pre>
    <div>
      ...
      <pre>&gt;</pre>
      <p>
        <span className="gold">☝️ Try to resize. It's responsive. Styles use plain CSS, not JS. </span>
        ...
        <span>
          <span className="nowrap">Pass a css string (to style something quickly, inline) - or a function</span>
          <span className="nowrap">that accepts theme as first argument and returns a string.</span> Use theme.instance to get color/shade/size of the current
          component.{" "}
        </span>
      </p>

      <Box as="p" ssIframe="display:none;">
        <span className="gold">⚠️ There used to be a paragraph in this spot. But now it's gone!</span> That was just an easy media query to check for iframe:{" "}
        <code>
      ...
      <Box as="p" ssNotIframe="display:none;">
        <span className="gold">😭 The values above may not reflect your true screen size... </span>
        <span>Because this content is inside an iframe. Width of the sidebar is subtracted. </span>
        <span>
          <a href="iframe.html?id=5--better-media-queries&viewMode=story" target="_blank" className="orange">
