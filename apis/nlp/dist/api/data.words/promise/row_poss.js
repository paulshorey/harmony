...
export default async function (row, options) {
    ...
    if (!tlds[row.key]) {
        ...
        if (row2 && row2.list_count && row2.pos1 === row.pos1 && row2.pos1 === row.pos1) {
             ...
             */
            if (DEBUG2)
                global.cconsole.log("row2 found", row2.key, row2.pos1);
            /*
             * combine counts
             */
            ...
            if (DEBUG2)
                global.cconsole.log("row2 NOT found", row.key, row.singular, row.plural, row.root);
        }
    }
    /*
     *
     ...
     */
    if (DEBUG2)
        global.cconsole.log("rebuild row.poss");
     ...
     */
    if (DEBUG2)
        global.cconsole.log("rebuild row.poss fix before/after");
    // before -
     ...
     */
    if (DEBUG2)
        global.cconsole.log("rebuild row.poss fix pos_short");
     ...
     */
    if (DEBUG2)
        global.cconsole.log("rebuild row.poss if proper/digit");
    ...
    if (DEBUG2)
        global.cconsole.log("rebuild row.poss return " + typeof row);
    return row;
}
