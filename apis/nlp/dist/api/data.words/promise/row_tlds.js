...
export default function (row) {
    return new Promise(async (resolve) => {
        ...
        if (DEBUG1)
            global.cconsole.warn(row.key, "from DB", row_tlds);
        /*
        ...
        let syns_checked = 0;
        if (DEBUG2)
            global.cconsole.log(`get key_tlds() for synonyms of key="${row.key}" row_tlds=`, row_tlds);
        ...
        for (let syn of [...row.poss[row.pos1].slice(0, 7)]) {
            ...
            // ILIKE matches in DB
            if (DEBUG2)
                global.cconsole.log(`get key_tlds() for syn="${syn}" of key="${row.key}"`);
            // make list from synonym
            let tlds = await key_tlds(syn);
            if (DEBUG2)
                global.cconsole.log(tlds);
        ...
        // time
        if (DEBUG_TIME)
            debug_time("everything until key_tlds");
        /*
         *
            ...
            for (let arr_i in arrs) {
                ...
                for (let tld of arr) {
                    ...
                    if (dict[tld]) {
                        ...
                        // log
                        if (DEBUG2)
                            global.cconsole.log("." + tld + " => category => ", arr);
                    }
                }
            }
        }
        if (DEBUG1)
            global.cconsole.log("after categories.reverse");
        /*
         *
        ...
        if (DEBUG1)
            global.cconsole.log("after abbreviations");
        /*
         * Combine, unique
         */
        if (DEBUG1) {
        ...
        // time
        if (DEBUG_TIME)
            debug_time("key_tlds " + syns_checked);
        /*
         *
         * Manual added TLDs
         *
        ...
        if (DEBUG1)
            global.cconsole.log("after manual tlds0");
        /*
         *
        ...
        if (DEBUG1)
            global.cconsole.log("after unshift derivations");
        /*
         *
        ...
        if (DEBUG1)
            global.cconsole.log("after sort");
        /*
         *
         * is Digit
         *
        ...
        // time
        if (DEBUG_TIME)
            debug_time("everything after key_tlds");
        if (DEBUG1)
            global.cconsole.warn(row.key, "final", row.tlds);
        resolve(row);
    });
}
