/*
 * IMPORTS
find ./common -depth -name "*.js" -exec sh -c 'mv "$1" "${1%.js}.ts"' _ {} \;
 */
...
export default function (expressApp, http_response) {
    ...
    expressApp.get("/api/data/word/:key", async function (req, res) {
        ...
        if (process.env._system_name !== "OSX") {
            if (DEBUG1)
                global.ccconsole.info(`GET "/api/data/word/${req.params.key}"`, { host: global["hostname"] });
            /*
             * Too many requests ?!?
             */
            requests_count++;
    ...
    expressApp.delete("/api/data/word/:key", async function (req, res) {
        ...
        if (process.env._system_name !== "OSX") {
            if (DEBUG1)
                global.ccconsole.info(`DELETE "/api/data/word/${req.params.key}"`, { host: global["hostname"] });
            /*
             * Too many requests ?!?
             */
            requests_count++;
    ...
    expressApp.put("/api/data/word_sentiment_of_synonym", async function (req, res) {
        ...
        if (process.env._system_name !== "OSX") {
            if (DEBUG1)
                global.ccconsole.info(`GET "/api/data/word_sentiment_of_synonym`, { host: global["hostname"] });
            /*
             * Too many requests ?!?
             */
            requests_count++;
