...
export default function (expressApp, http_response) {
    ...
    expressApp.all("/v1/*availability", async function (req, res) {
        ...
        if (DEBUG2)
            global.cconsole.log("/v1/availability", req.query, req.body);
        // aggregate POST data and URL parameters
        let query = aggregate_req_body_query(req);
        if (DEBUG2)
            global.cconsole.log("/v1/availability", query);
        if (query) {
        ...
        if (DEBUG1)
            global.cconsole.log("/v1/availability", query.domains.length);
        ...
        try {
             ...
             */
            if (DEBUG_ENDPOINTS)
                global.cconsole.info("GET/v1/availability " + query.domains.length);
             ...
             */
            if (DEBUG_TIME)
                global.cconsole.info("availability " + Math.round(performance.now() - start_time) + " ms");
            ...
            if (DEBUG_TIME)
                global.cconsole.warn("availability " + Math.round(performance.now() - start_time) + " ms");
