...
export default function (domstr, options = {}) {
    return new Promise((resolve) => {
        ...
        global.execute("whois " + domstr, (whois) => {
            ...
            if (DEBUG2)
                console.log("whois results: ", domstr, whois);
            let output = { dom: domstr, code: 0 };
            ...
            try {
                ...
                }
                {
                    ...
                    if (matches) {
                        for (let line of matches) {
                            if (DEBUG1)
                                console.log("changeds", line);
                            let matched = line.match(/.*?change.*?([0-9]{4})(.*)/);
                            if (matched) {
                                if (DEBUG1)
                                    console.log("changed", matched);
                ...
                }
                {
                    ...
                    if (matches) {
                        for (let line of matches) {
                            if (DEBUG1)
                                console.log("expiress", line);
                            let matched = line.match(/.*?expir.*?([0-9]{4})(.*)/);
                            if (matched) {
                                if (DEBUG1)
                                    console.log("expires", matched);
                ...
                if (output.ns) {
                    ...
                    else if (output.ns.includes(".dan.") ||
