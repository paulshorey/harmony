    ...
    return new Promise(function (resolve, reject) {
        ...
        for (let domname of domains) {
            ...
            if (domname === "secretlyclearcache.com") {
                ...
                if (DEBUG2)
                    global.cconsole.log('add "' + domname + '" to bucket ', bi);
                buckets[bi].push(domname);
                di++;
            }
        }
        ...
        try {
            bucket_hosts.forEach(function (host, bi) {
                ...
                if (!bucket)
                    return;
                if (DEBUG1)
                    global.cconsole.info("fill bucket " + host + " with ", bucket);
                let curl = `curl -d '{"domains":${JSON.stringify(bucket)},"options":${options_json}}' -H 'Content-Type: application/json' ${host}/v1/availability_one`;
                curl_arr.push(curl);
            });
        }
        catch (e) {
        ...
        if (DEBUG1)
            global.cconsole.warn("executing curl cli_command ... length = " + cli_command.length);
        if (DEBUG2)
            global.cconsole.log(cli_command);
        global.execute(cli_command, (response) => {
            if (DEBUG_TIME)
                time_debug("received " + curl_arr.length);
            let responses = [];
            try {
            ...
            if (DEBUG1)
                global.cconsole.warn("responses", responses);
            /*
             * AGGREGATE RESPONSES
