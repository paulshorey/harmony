...
export default async function (req, { allow_trusted_bypass = true, BYPASS_AUTH = false } = {}) {
    ...
    if (!user.expires) {
        if (BYPASS_AUTH || (allow_trusted_bypass && is_auth_ip_current(req.client_ip))) {
             ...
             */
            if (DEBUG1)
                global.cconsole.info("trusted req.client_ip", req.client_ip);
        }
        else {
            /*
             *
             ...
             */
            if (DEBUG1)
                global.cconsole.info("NOT trusted client ip, validate captcha2", req.client_ip);
            /*
             * captcha2 or captcha3 is required at this point
             */
            let input_recaptcha2_token = (req.body && req.body.recaptcha2_token) ||
                (req.query.recaptcha2_token ? decodeURIComponent(req.query.recaptcha2_token) : "") ||
                "";
            let input_recaptcha3_token = (req.body && req.body.recaptcha3_token) ||
            ...
            if (!input_recaptcha2_token && !input_recaptcha3_token) {
                ...
                if (captcha_auth.data.success) {
                     ...
                     */
                    if (DEBUG1)
                        global.ccconsole.log("captcha_auth.data.success", req.client_ip);
