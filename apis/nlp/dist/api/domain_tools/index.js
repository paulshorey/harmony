...
export default function (expressApp, http_response) {
     ...
     */
    {
        ...
        expressApp.get(url, async function (req, res) {
            if (DEBUG1)
                global.cconsole.info("/v1/whois", req.query, req.body);
            ...
            try {
                 ...
                 */
                if (DEBUG_TIME)
                    global.cconsole.info(url + " " + Math.round(performance.now() - start_time) + " ms");
                ...
                if (DEBUG_TIME)
                    global.cconsole.warn(url + " " + Math.round(performance.now() - start_time) + " ms");
