...
export default async function (output, options = {}) {
    if (!output || !output.string || !output.chunks_keys || !output.chunks_rows)
        return;
    output.chunks_info = {};
    for (let key in output.chunks_rows) {
        ...
        if (row) {
            ...
            if (row.pos1 && row.poss[row.pos1])
                info[pos_expand[row.pos1]] = row.poss[row.pos1];
            if (row.pos2 && row.poss[row.pos2])
                info[pos_expand[row.pos2]] = row.poss[row.pos2];
            if (row.pos3 && row.poss[row.pos3])
                info[pos_expand[row.pos3]] = row.poss[row.pos3];
