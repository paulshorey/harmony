...
export const data_interactions_get_all = function () {
    return new Promise(async (resolve) => {
        ...
        if (DEBUG2)
            global.cconsole.log("get queryString:", queryString);
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const data_interactions_get_all_parsed = function () {
    return new Promise(async (resolve) => {
        let rows = await data_interactions_get_all();
        if (DEBUG1)
            global.cconsole.log("rows distinct?", rows);
        if (!rows || !rows.length) {
            resolve([]);
        }
        for (let ri in rows) {
...
export const data_interactions_get_all_distinct_parsed = function () {
    return new Promise(async (resolve) => {
        let rows = await data_interactions_get_all_distinct();
        if (DEBUG1)
            global.cconsole.log("rows distinct?", rows);
        if (!rows || !rows.length) {
            resolve([]);
...
export const data_interactions_put = function (row) {
    if (DEBUG2)
        global.cconsole.log("data_interactions_put", JSON.stringify(row).substr(0, 100));
    if (DEBUG1)
        global.cconsole.log("data_interactions_put", row.email);
    return new Promise(async (resolve) => {
        // required inputs
        if (!row.email) {
            resolve("!row.email");
