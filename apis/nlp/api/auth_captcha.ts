 ...
 */
export default async function (req, { allow_trusted_bypass = true, BYPASS_AUTH = false }: any = {}) {
  let user: any = {}
    ...
    if (BYPASS_AUTH || (allow_trusted_bypass && is_auth_ip_current(req.client_ip))) {
      ...
      if (!input_recaptcha2_token && !input_recaptcha3_token) {
        ...
        if (captcha_auth.data.success) {
           ...
           * captcha success
           */
          if (DEBUG1) global.ccconsole.log("captcha_auth.data.success", req.client_ip)
