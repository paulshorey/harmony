...
export default function (doms_arr, { DEBUG_TIME = false } = {}) {
  return new Promise((resolve) => {
    ...
    if (doms_length > MAX_DOMS) {
      global.ccconsole.error("received too many domains. limiting to " + MAX_DOMS)
    ...
    axios
      ...
      .then((response) => {
        ...
        for (let item of response.data.results) {
          ...
          if (DEBUG1) global.ccconsole.info(dom, item)
        ...
        if (DEBUG1) {
          global.ccconsole.info("doms output", doms_dict)
        }
