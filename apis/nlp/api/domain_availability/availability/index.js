...
const availability = async function ({ domains = [], options }) {
  ...
  for (let domname of domains) {
    ...
    // if (b_name.length === 50) {
    //   if (DEBUG1) global.ccconsole.log("fill bucket name", 50, b_name)
    //   requests.push(check_name(b_name, { DEBUG_TIME }))
    //   b_name = [] // start over - empty
    // }
    // dump b_domainr bucket (is full)
    if (b_domainr.length === 50) {
      if (DEBUG1) global.ccconsole.log("fill bucket domainr", 50, b_domainr)
  ...
  // dump remaining content in bucket (what's left after last, full dump)
  {
    ...
    if (len) {
      if (DEBUG1) global.ccconsole.log("fill bucket cli_host", len)
  ...
  //   if (len) {
  //     if (DEBUG1) global.ccconsole.log("fill bucket name", len)
  //     requests.push(check_name(b_name, { DEBUG_TIME }))
  //   }
  // }
  {
  ...
  }
  {
    ...
    if (len) {
      if (DEBUG1) global.ccconsole.log("fill bucket cli_whois", len)
  ...
  }
  {
    ...
    if (len) {
      if (DEBUG1) global.ccconsole.log("fill bucket domainr", len)
      requests.push(check_domainr(b_domainr, { DEBUG_TIME }))
    }
  }
   ...
   * LOG
   */
  if (DEBUG1) global.ccconsole.log("n_from_localstorage", n_from_localstorage)
