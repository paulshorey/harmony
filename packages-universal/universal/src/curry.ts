 ...
 * ` let toDoLater = call_later(console.warn, 'go for a run'); `
 * @param {function} func - function you want to call later (to curry). It can be bound
 * @param {array} args - array of arguments (this curry is not useful without arguments)
 */
export function call_later(func: Function, args: Array<any>): Function {
  return function () {
    return func(...args);
  };
}
