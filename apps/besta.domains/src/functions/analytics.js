import { getAttributionParams, href_canonical } from 'src/functions/url';
...
export const analytics_track_page = function (options) {
  // Mixpanel
  // always track with a trailing slash
  options.path = options.path + '/';
  // remove redundant trailing slash if path already had one
  options.path = options.path.replace(/\/\/$/, '/');
  // attribution parameters from url and from page data (passed down in options argument)
  let qs = getAttributionParams(options);
  options.referralCode = qs.referralCode;
  options.source = qs.source;
  options.ein = qs.ein;
  options.pid = qs.pid;
  options.c = qs.c;
...
const href_type = function (href) {
  ...
  if (href.substring(0, 1) === '#') {
    ...
    !/spiral\.us|localhost:/.test(href.substring(0, 22))
  ) {
    prefix = 'external';
