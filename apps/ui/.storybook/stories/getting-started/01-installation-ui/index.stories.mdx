...
<Meta title="Get started/Theme provider" />
...
This is **not currently published as a standalone library**, but as part of a **mono-repo**.  

See 👈 ["Mono repo" in the stories panel](/docs/get-started-mono-repo--page).   

This UI library `@ps/ui` by design only requires `@emotion/core` and `@emotion/react` as peer dependencies.   
As it's configured now, it also uses a couple other packages in this mono-repo `@ps/fn` and `@ps/contants`.  


<Block as="h2" textgradient="accent" ss="margin-top:3rem;">2. Theme</Block>

To get started instantly, with no configuration, import `@ps/ui/components/utils/ThemeProvider` and wrap your app with it.

Or to customize theme and settings, make a copy of `@ps/ui/components/utils/ThemeProvider` in your app directory. Import that and wrap your app in that. Same with JS and CSS files it imports. Customize anything.  

Here are the files you need to copy and edit to customize your colors, sizes, and variables:

<Code code={`// ThemeProvider.tsx
  
import { ThemeProvider as EmotionThemeProvider } from '@emotion/react';
  
import theme from 'styles/theme'; // your custom JS variables
  
import 'styles/theme.css'; // your custom CSS variables
...
import mq from './mq';  // copy and customize these too if you want
