...
<Inline as="b" textcolor="accent">
  🤓 The old and new props will be combined.
...
- the output of `css` template literal function from `styled-components`
...
SS props are NOT the same as styled-components template literal functions. So, your IDE will not color these like CSS. UNLESS...
...
<Block as="p" ss={css\`
  /* THIS WILL BE syntax-HIGHLIGHTED by vscode-styled-components plugin (and others)! */
\`}>
...
Not even when wrapping it in `css` from `styled-components`...
...
Different format than the regular `styled-components`, but gets the job done.  
