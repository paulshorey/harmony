...
<Meta title="Get started/Styling system/SS Props vs. Styled Components" />

<ThemeProvider>
<Wrapper>

# There are a few ways to use these components:

<br />

<Block as="h2" ss="margin-top: 1rem;">1. The default export is ready to use</Block>

All components in this library accept `ss` props, which accept a string of SCSS or a function that returns a string.
<Code
  ss="font-size: 0.85rem;"
  code={`import { css } from '@emotion/react'; // optional
...
<Button 
  ...
  onClick={console.log} 
  ss={(props) => css\`
      /* pass a function to any ss prop to access props and variables */
      \${props.theme.mq.desktopLarge} {
          /* use props and theme variables, even inside media queries */
          padding: 1rem \${props.theme.sizes.card.paddingX * 2};
      }
      /* optionally: wrap it a css function to add syntax highlighting */
  \`}
  ssDesktopLarge="/* See Media queries to learn about all ss props. */"
>
  Click me
</Button>`}
/>
...
This library uses `@emotion/styled` instead of `styled-components`. It's the same thing, but with extra features. Both are great libraries. You can literally pass anything to styled! Behind the scenes it simply adds a className to the component, then writes the CSS to a global stylesheet. That's also how this library's `ss` prop works.
