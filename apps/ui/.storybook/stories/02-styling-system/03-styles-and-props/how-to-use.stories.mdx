...
All components in this library accept `ss` props, which accept a string of SCSS or function that returns a string.
<Code
...
<Button 
  ...
  \`}
  ssDesktopLarge="/* Or use the same media queries as a shortcut prop. */"
>
  Click me
</Button>`}
/>

<Block as="h2" ss="margin-top: 3.5rem;">2. Provided HOC is like Styled Components</Block>
...
This library uses `@emotion/styled` instead of `styled-components`. It's the same thing, but with extra features. 
...
**If you like method #2 above**, it works with styled components too. Styled is a great library. You can literally pass anything to it! Behind the scenes, it just adds a className to whatever component you pass to it.
