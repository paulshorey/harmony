...
export default function (expressApp, http_response) {
  ...
  expressApp.all("/v1/*availability", async function (req, res) {
    ...
    if (DEBUG2) cconsole.log("/v1/availability", req.query, req.body)
    // aggregate POST data and URL parameters
    let query = aggregate_req_body_query(req)
    if (DEBUG2) cconsole.log("/v1/availability", query)
    if (query) {
      if (query.domains && typeof query.domains === "string") {
        query.domains = query.domains.split(",")
      }
    }
    if (DEBUG1) cconsole.log("/v1/availability", query.domains.length)
    ...
    try {
       ...
       * get availability
       */
      if (DEBUG_ENDPOINTS) cconsole.info("GET/v1/availability " + query.domains.length)
       ...
       * return data
       */
      if (DEBUG_TIME) cconsole.info("availability " + Math.round(performance.now() - start_time) + " ms")
      ...
      ccconsole.error("GET/POST /v1/availability", err)
      if (DEBUG_TIME) cconsole.warn("availability " + Math.round(performance.now() - start_time) + " ms")
