...
export default function (doms_arr, { DEBUG_TIME = false } = {}) {
  return new Promise((resolve) => {
    ...
    if (!doms_length) {
      ccconsole.warn("empty list of input doms gets empty list of results")
      resolve(doms_dict)
    }
    if (doms_length > MAX_DOMS) {
      ccconsole.error("received too many domains. limiting to " + MAX_DOMS)
    ...
    axios
      .post(url, data)
      .then((response) => {
        if (DEBUG1) cconsole.log(typeof response.data.results, response.data.results)
        ...
        for (let item of response.data.results) {
          ...
          if (DEBUG1) ccconsole.info(dom, item)
        ...
        if (DEBUG1) {
          ccconsole.info("doms output", doms_dict)
        }
