// import check_ping from "api/domain_availability/availability/promise_one/cli_host"
...
export default function (doms_arr, { DEBUG_TIME = false } = {}) {
  return new Promise(async (resolve) => {
    ...
    cli_command += "} | cat"
    if (DEBUG1) cconsole.log(cli_command)
    ...
    global.execute(cli_command, (ping_response) => {
      ...
      for (let line of arr) {
        ...
        if (DEBUG1) cconsole.log(dom, code)
      ...
      // time
      if (DEBUG_TIME) {
        cconsole.info('cli_host length="' + dom_nums + '" time="' + (Date.now() - start_time) + ' ms"')
      }
