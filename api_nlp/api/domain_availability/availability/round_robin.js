...
export default function ({ domains = [], options = {} }) {
  return new Promise(function (resolve, reject) {
    // debug time
    let time_start = Date.now()
    let time_debug = function (message = "") {
      if (DEBUG_TIME) {
        cconsole.info("TIME /round_robin", ((Date.now() - time_start) / 1000).toFixed(3), message)
      }
    }
    ...
    for (let domname of domains) {
      ...
      if (domname === "secretlyclearcache.com") {
        ...
        if (DEBUG2) cconsole.log('add "' + domname + '" to bucket ', bi)
        buckets[bi].push(domname)
        di++
      }
    }
    ...
    try {
      bucket_hosts.forEach(function (host, bi) {
        ...
        if (!bucket) return
        if (DEBUG1) cconsole.info("fill bucket " + host + " with ", bucket)
    ...
    if (DEBUG1) cconsole.warn("executing curl cli_command ... length = " + cli_command.length)
    if (DEBUG2) cconsole.log(cli_command)
    global.execute(cli_command, (response) => {
      ...
      if (DEBUG1) cconsole.warn("responses", responses)
