import check_domainr from "api/domain_availability/availability/promise_async/domainr.js"
// import check_name from "api/domain_availability/availability/promise_many/name.js"
// import check_moi from "api/domain_availability/availability/promise_many/moi.js"
import check_cli_host from "api/domain_availability/availability/promise_many/cli_host.js"
// import check_whoisxmlapi from "api/domain_availability/availability/promise_many/whoisxmlapi.js"
...
const availability = async function ({ domains = [], options = {} }) {
  ...
  let time_debug = function (message = "") {
    if (DEBUG_TIME) {
      cconsole.info("DEBUG_TIME /availability_one", ((Date.now() - time_start) / 1000).toFixed(3), message)
    }
  }
  ...
  for (let domname of domains) {
    ...
    //   let row = await data_word_get(domsld,'list_count')
    ...
    if (domsld === "secretlyclearcache") {
      cconsole.warn("localStorage.clear()")
      localStorage.clear()
      let row = {
  ...
  // dump remaining content in bucket (what's left after last, full dump)
  {
    ...
    if (len) {
      if (DEBUG1) ccconsole.log("fill bucket cli_host", len)
  ...
  //   if (len) {
  //     if (DEBUG1) ccconsole.log("fill bucket name", len)
  //     requests.push(check_name(b_name, { DEBUG_TIME }))
  //   }
  // }
  {
  ...
  }
  {
    ...
    if (len) {
      if (DEBUG1) ccconsole.log("fill bucket cli_whois", len)
  ...
  }
  {
    ...
    if (len) {
      if (DEBUG1) ccconsole.log("fill bucket domainr", len)
      requests.push(check_domainr(b_domainr, { DEBUG_TIME }))
    }
  }
   ...
   * LOG
   */
  if (DEBUG1) ccconsole.log("n_from_localstorage", n_from_localstorage)
