 ...
 * IMPORTS
 */
import { data_interactions_get_all_parsed, data_interactions_put } from "api/users.interactions/pgdb.js"
...
export default function (expressApp, http_response) {
   ...
   * GET interactions
   */
  expressApp.get("/v1/interactions", async function (req, res) { global.res = res
    if (DEBUG1) cconsole.info("GET /interactions")
    // http_response(res, 500, "idk")
    try {
      /*
       * get data
       */
      let rows = await data_interactions_get_all_parsed()
      if (!rows) {
      ...
      ccconsole.error("GET /v1/interactions", err)
   ...
   * PUT interactions
   */
  expressApp.put("/v1/interactions", async function (req, res) { global.res = res
    if (DEBUG1 && !DEBUG2) cconsole.info("PUT /interactions")
    if (DEBUG2) cconsole.log("PUT /interactions body", req.body)
    // http_response(res, 500, "idk")
    try {
      if (!req.body.contact) {
      ...
      let error = await data_interactions_put(row)
      if (error) {
      ...
      ccconsole.error("PUT /v1/interactions", err)
