...
export const data_interactions_get_all = function () {
  return new Promise(async (resolve) => {
    ...
    if (DEBUG2) cconsole.log("get queryString:", queryString)
    pool
      .query(queryString)
      .then((res) => {
        if (res.rows.length) {
...
export const data_interactions_get_all_parsed = function (columns, limit) {
  return new Promise(async (resolve) => {
    ...
    if (DEBUG1) cconsole.log("rows distinct?", rows)
    if (!rows || !rows.length) {
      resolve([])
    }
    for (let ri in rows) {
...
export const data_interactions_put = function (row) {
  if (DEBUG2) cconsole.log("data_interactions_put", JSON.stringify(row).substr(0, 100))
  if (DEBUG1) cconsole.log("data_interactions_put", row.email)
  return new Promise(async (resolve) => {
    // required inputs
    if (!row.email) {
      resolve("!row.email")
    ...
    try {
      ...
      pool
        ...
        .catch((e) => {
          cconsole.warn("pg pool failed", e.message || e)
          resolve(e.message || e)
        })
    } catch (e) {
