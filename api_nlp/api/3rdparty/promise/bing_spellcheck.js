...
export default function (string, options={}) {
  return new Promise(function (resolve) {
    ...
    let timeout = setTimeout(
      () => {
        cconsole.warn("TIMEOUT SPELLCHECK")
    ...
    let time_debug = function () {
      if (options.DEBUG_TIME) {
        cconsole.info("DEBUG_TIME SPELLCHECK", ((performance.now() - time_start) / 1000).toFixed(2))
      }
    }
    ...
    setImmediate(function () {
      // start
      if (DEBUG1) cconsole.info(`spellcheck input "${string}"`)
      ...
      try {
        ...
        // send request
        if (DEBUG2) cconsole.log("send response", data)
        axios
          .get(url, {
            params: data,
            headers: headers,
          ...
          .then(function (response) {
            if (DEBUG2) cconsole.log("got response", response.data)
            // response data
            if (response.data.flaggedTokens[0]) {
              ...
              if (DEBUG1) cconsole.log("bing suggestions", suggestions)
            ...
            if (DEBUG1) {
              if (string_spellchecked && string_spellchecked !== string) {
                cconsole.log(`bing spellcheck string_spellchecked "${string}" -> "${string_spellchecked}"`)
              } else {
                if (DEBUG1) cconsole.info(`bing spellcheck did not fix "${string}"`)
              }
            }
          ...
          .catch(function (error) {
            if (DEBUG1) ccconsole.warn(`bing spellcheck failed`, error)
            clearTimeout(timeout)
            resolve(string)
          })
      } catch (err) {
        if (DEBUG1) ccconsole.error(`bing spellcheck error on "${string}"`, err)
      }
    })
  })
}
