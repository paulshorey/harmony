...
export default function (string, options = {}) {
  return new Promise(function (resolve) {
    ...
    let timeout = setTimeout(
      () => {
        cconsole.warn("TIMEOUT GAUTOCOMPLETE")
    ...
    let time_debug = function () {
      if (options.DEBUG_TIME) {
        cconsole.info("DEBUG_TIME GAUTOCOMPLETE", ((performance.now() - time_start) / 1000).toFixed(2))
      }
    }
    ...
    setImmediate(function () {
      // start
      if (DEBUG1) cconsole.info(`GAutocomplete input "${string}"`)
      let output = {
        source: "google"
      }
      let suggestions = []
      try {
        ...
        axios
          ...
          .then(function (response) {
            ...
            if (response.data && response.data[1] && response.data[1] instanceof Array) {
              ...
              if (DEBUG1) cconsole.log("GAutocomplete all", suggestions)
            ...
            if (DEBUG1) cconsole.log("GAutocomplete filtered", suggestions)
        ...
        if (DEBUG1) ccconsole.error(`GAutocomplete error on "${string}"`, err)
      }
    })
  })
}
