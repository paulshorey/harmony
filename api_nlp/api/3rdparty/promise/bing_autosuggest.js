...
export default function (string, options = {}) {
  return new Promise(function (resolve) {
    ...
    let timeout = setTimeout(
      () => {
        cconsole.warn("TIMEOUT AUTOSUGGEST")
    ...
    let time_debug = function () {
      if (options.DEBUG_TIME) {
        cconsole.info("DEBUG_TIME AUTOSUGGEST", ((performance.now() - time_start) / 1000).toFixed(2))
      }
    }
    ...
    setImmediate(function () {
      // start
      if (DEBUG1) cconsole.info(`autosuggest input "${string}"`)
      let output = {
        source: "bing"
      }
      let suggestions = []
      try {
        ...
        axios
          ...
          .then(function (response) {
            ...
            if (
              ...
              if (DEBUG1) cconsole.log("all", suggestions)
            ...
            if (DEBUG1) cconsole.log("filtered", suggestions)
            ...
            // // only use last words
            // output.alts = []
            // if (DEBUG1) cconsole.log("use bing astr:", astr)
            // if (DEBUG1) cconsole.log("use bing suggestions:", suggestions)
        ...
        if (DEBUG1) ccconsole.error(`bing autosuggest error on "${string}"`, err)
      }
    })
  })
}
