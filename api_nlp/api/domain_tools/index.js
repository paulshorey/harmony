...
export default function (expressApp, http_response) {
   ...
   */
  {
    ...
    expressApp.get(url, async function (req, res) {
      ...
      if (DEBUG1) cconsole.info("/v1/whois", req.query, req.body)
      ...
      try {
         ...
         * return data
         */
        if (DEBUG_TIME) cconsole.info(url + " " + Math.round(performance.now() - start_time) + " ms")
        http_response(res, 200, results)
      } catch (err) {
        err.user_id =
          req.headers["x-real-ip"] || req.headers["x-forwarded-for"] || req.ip || req.connection.remoteAddress
        ccconsole.error("GET " + url, err)
        if (DEBUG_TIME) cconsole.warn(url + " " + Math.round(performance.now() - start_time) + " ms")
