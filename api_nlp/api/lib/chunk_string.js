...
import wordchunk from "api/lib/wordchunk"
import row_model from "api/data.words/function/row_model"
...
export default async function (input_str, tld, do_spellcheck) {
  if (DEBUG1) cconsole.log('api/lib/chunk_string', input_str, tld);
  let output_arr
  let output_alts
  ...
  if (DEBUG1) cconsole.log('AFTER wordbreak(), output_str = ', output_str);
  // parse str (if result is different than input, skip next step)
  // if (do_spellcheck) {
  let spelled_obj = await spellcheck(output_str)
  if (DEBUG1) cconsole.log('AFTER spellcheck(), spelled_obj = ', spelled_obj);
  if (spelled_obj) {
  ...
  if (DEBUG1) cconsole.log('AFTER wordbreak(), output_arr = ', output_arr);
