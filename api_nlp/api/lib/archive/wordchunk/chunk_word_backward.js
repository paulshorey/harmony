...
const chunk_word_backward = async function (string, options = {}) {
  if (DEBUG2) cconsole.log('chunk backward "' + string + '"')
  /*
   * validate & filter
   */
  // set default options:
  ...
  if (DEBUG1) cconsole.info("chunk_word_backward()", string)
  ...
  for (let i_to = string.length; i_to >= MINLEN; i_to--) {
    ...
    if (DEBUG2) cconsole.log(str, row ? row.list_count : "row is undefined")
    // not real word?
    ...
    if (DEBUG2) cconsole.success("found chunk row", str, row.list_count, row.pos1, row.aux)
    ...
    //   if (DEBUG1)
    //     cconsole.warn(`found "${row.key}"... now check -1`, row.aux, row.pos1, row.list_count, row.pos1, row.aux)
    ...
    //         if (DEBUG1) cconsole.warn(`found -1 "${str2}"`, row2.list_count)
    ...
    //           if (DEBUG2) cconsole.warn(`str2="${row2.key}" pos="${row2.pos1}" count="${row2.list_count}"`)
    //           if (DEBUG2) cconsole.warn(`is better than str="${row.key}" pos="${row.pos1}" count="${row.list_count}"`)
    //           continue
    //         }
    //       }
    //     }
    ...
    if (str_remainder) {
      ...
      if (row_remainder && row_remainder.list_count >= options.MINSYN) {
        if (DEBUG2) cconsole.warn("remainder chunk row", str_remainder, row_remainder.list_count)
