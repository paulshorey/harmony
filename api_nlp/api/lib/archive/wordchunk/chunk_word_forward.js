...
const chunk_word_forward = async function (string, options = {}) {
  if (DEBUG2) cconsole.log('chunk forward "' + string + '"')
  /*
   * validate & filter
   */
  // set default options:
   ...
   * chunk string
   */
  if (DEBUG1) cconsole.info("chunk_word_forward()", string)
  ...
  for (let i_from = 0; i_from <= i_to - MINLEN; i_from++) {
    ...
    //         if (DEBUG2) cconsole.warn(`str2="${row2.key}" pos="${row2.pos}" count="${row2.list_count}"`)
    //         if (DEBUG2) cconsole.warn(`is better than str="${row.key}" pos="${row.pos}" count="${row.list_count}"`)
    //         continue
    //       }
    //     }
    //   }
