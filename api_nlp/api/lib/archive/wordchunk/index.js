...
const thisModule = async function (string, super_options = {}) {
  ...
  if (DEBUG1) cconsole.info(`chunk_word() "${string}"`)
  let MINSYN = 4
  let options = {
    str_row_options : {
      model: false,
  ...
  try {
    ...
    if (DEBUG1) cconsole.warn(string, Object.keys(options.chunked_rows), options.found_backward, options.found_forward)
    ...
    if (options.found_backward.length || options.found_forward.length) {
      ...
      if (DEBUG0) cconsole.warn(`                   ch1_str "${ch1_str}"`)
    ...
    if (DEBUG1) cconsole.debug(`after 1st try ((${ch1_score}))`, ch1_keys)
    if (DEBUG1) cconsole.debug(`str_forward="${ch1_forward_key}"`)
    if (DEBUG1) cconsole.debug(`str_backward="${ch1_backward_key}"`)
    ...
    if (DEBUG0) cconsole.warn(`                   ch2_str "${ch2_str}"`)
    ...
    if (DEBUG1) cconsole.debug(`after 2nd try ((${ch2_score}))`, ch2_keys)
    if (DEBUG1) cconsole.debug(`str_forward="${ch2_forward_key}"`)
    if (DEBUG1) cconsole.debug(`str_backward="${ch2_backward_key}"`)
    // }
    ...
    if (!ch1_str_changed && ch2_string_changed) {
       ...
       * use #2
       */
      if (DEBUG1) cconsole.log("!ch1_str_changed && ch2_string_changed", Object.keys(ch1_rows))
      return { chunked_string: ch1_str, chunked_rows: ch1_rows }
    } else if (ch1_str_changed && !ch2_string_changed) {
      /*
       * use #2
       */
      if (DEBUG1) cconsole.log("ch1_str_changed && !ch2_string_changed", Object.keys(ch2_rows))
      ...
      if (ch1_score > ch2_score) {
        // choose 1st
        if (DEBUG1) cconsole.log("ch1_score > ch2_score", Object.keys(ch1_rows))
        return { chunked_string: ch1_str, chunked_rows: ch1_rows }
      } else {
        // choose 2nd
        if (DEBUG1) cconsole.log("ch1_score <= ch2_score", Object.keys(ch2_rows))
        return { chunked_string: ch2_str, chunked_rows: ch2_rows }
      }
    } else {
      /*
       ...
       */
      if (DEBUG1) cconsole.error("possibly_corrupted_output", Object.keys(ch2_rows))
    ...
    //   if (DEBUG1) cconsole.log(`found str_backward "${options.str_backward}"`)
    ...
    //   if (DEBUG1) cconsole.log(`found str_forward "${options.str_forward}"`)
    ...
    //   if (DEBUG1)
    //     cconsole.log(`of remainder_backward "${options.remainder_backward}" str_forward "${options.str_forward}"`)
    ...
    //   }
    // }
    // if (DEBUG1) cconsole.log([key_first, key_middle, key_last])
    ...
    //   } else {
    //     // can't parse!!!
    //     cconsole.error(`cant parse: "${string}", 2nd if else`)
    //   }
    // } else if (row_first && !row_last) {
    //   final_keys = [key_first, key_middle + key_last]
    ...
    // } else {
    //   // can't parse!!!
    //   cconsole.error(`cant parse: "${string}", 1st if else`)
    // }
    //
    // // cconsole.log("final_keys", final_keys)
    // //
    // // build string/dict
    ...
    ccconsole.error("chunk_word error: " + e.toString())
  }
}
