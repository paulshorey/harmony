...
let debug_time = function (message) {
  ...
  if (debug_time_since && message) {
    cconsole.log(`DEBUG_TIME ${message} = `, Math.round((time - debug_time_since) / 10) / 100)
  }
...
const mymodule = async function (string, tld = "com") {
  ...
  if (d1 && !isNaN(d1.index)) {
    for (let str of d1) {
      ...
      if (DEBUG1) cconsole.warn(`adding space to string "${string}" at index "${d1_i_start}"`)
      d1_i_start++
      ...
      if (w) {
        if (w.index > 0) {
          ...
          if (DEBUG1) cconsole.warn(`adding space to string "${string}" at index "${w.index}"`)
        }
      }
    }
  }
  ...
  if (DEBUG2) cconsole.debug("chunks after original", chunks_keys)
  ...
  if (!row_original || !row_original.list_count || row_original.list_count <= 15) {
    ...
    if (DEBUG3) cconsole.warn("found spellcheck string = " + found.spellchecked)

    // alt chunk
    let row = await str_row(string, { REBUILD: REBUILD, model: false })
    if (DEBUG3 && row) {
      cconsole.warn("found spellcheck row = ", {
        key: row.key,
  ...
  if (DEBUG3 || DEBUG_TIME) {
    cconsole.info("DEBUG TIME SPELLCHECK/AUTOSUGGEST", ((Date.now() - time_spellchecked) / 1000).toFixed(2))
  }
  ...
  if (DEBUG2) cconsole.debug("chunks after wordbreak", chunks_keys)
  ...
  if (string_arr.length > 1 && chunks_keys.length <= 1) {
    ...
    for (let key of string_arr) {
      ...
      // if (DEBUG2) cconsole.warn("key, row", key, Object.keys(row))
      if (row && row.key) {
  ...
  if (DEBUG2) cconsole.debug("chunks after highest resolution", chunks_keys)
  ...
  if (string_arr.length >= 3) {
    ...
    for (let i_to = string_arr.length - 1; i_to > 1; i_to--) {
      ...
      if (DEBUG2) cconsole.info(`middle resolution backward = "${key}"`)
      let row = await str_row(key, { REBUILD: REBUILD })
      // save only if its a good phrase! if enough synonyms
      if (row && row.list_count >= 10) {
        if (DEBUG1) cconsole.success("middle resolution backward - added row", key)
        /*
         * found subphrase!
         */
        ...
        chunks_keys.splice(chunks_keys - 1, 0, string_arr)
        chunks_rows[key] = row
      }
    }
    // search forward
    ...
    for (let i_from = 1; i_from < string_arr.length - 1; i_from++) {
      let words_slice = string_arr.slice(i_from, i_to)
      let key = words_slice.join(" ").trim()
      if (DEBUG2) cconsole.info(`middle resolution forward = "${key}"`)
      let row = await str_row(key, { REBUILD: REBUILD })
      // save only if its a good phrase! if enough synonyms
      if (row && row.list_count >= 10) {
        if (DEBUG1) cconsole.success("middle resolution forward - added row", key)
        /*
         * found subphrase!
         */
        // if (row.list_count >= 25) {
  ...
  if (DEBUG2) cconsole.debug("after middle resolution", string, chunks_keys)
  // debug
  if (DEBUG2) cconsole.debug("chunks_keys", chunks_keys)
  if (DEBUG2) cconsole.debug("string ", string)
  if (DEBUG2) cconsole.debug("string_arr ", string_arr)
  if (DEBUG2) cconsole.debug("chunks_rows ", Object.keys(chunks_rows))
  ...
  if (DEBUG1) cconsole.warn("final chunks output chunks_keys", chunks_keys)
  if (DEBUG1) cconsole.warn("final chunks output string_arr", string_arr)
