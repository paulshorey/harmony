...
export default function (domain, tlds_ratings) {
  ...
  if (domain.words[2] && domain.words[0] === domain.words[1]) {
    if (DEBUG3) ccconsole.warn("first words the same", domain)
    return -10000
  }
  let last_i = domain.words.length - 1
  if (domain.words[last_i] && domain.words[last_i - 1] === domain.words[last_i]) {
    if (DEBUG3) ccconsole.warn("last words the same", domain)
    return -10000
  }
  let rating = 0
  ...
  // if (tld === "com") {
  //   // cconsole.log(domain.string, n_words, ((n_words-1)*(n_words-1)*(n_words-1)*50))
