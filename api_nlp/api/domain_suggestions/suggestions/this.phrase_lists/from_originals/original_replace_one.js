...
export default function () {
  ...
  for (let chunk_i = 0; chunk_i < use_keys.length; chunk_i++) {
    ...
    if (DEBUG1) cconsole.info(row.key, row.pos1, !!row.poss[row.pos1])
    ...
    if (row.poss[row.pos1]) {
      ...
      if (DEBUG1) cconsole.log("use synonyms", row.poss[pos])
   ...
   *
   */
  if (DEBUG0) cconsole.log("lists_of_phrases", lists_of_phrases)
  ...
  if (DEBUG0) cconsole.log(this.phrase_lists["replace one"])
}
