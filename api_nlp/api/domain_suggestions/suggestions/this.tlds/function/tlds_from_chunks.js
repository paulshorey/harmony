...
export default async function() {
  ...
  let dset5 = new Set()
  if (DEBUG1) {
    cconsole.log("this.keys_words", this.keys_words)
    cconsole.log(
  ...
  if (DEBUG3) cconsole.debug("ndoms", ndoms)
  if (DEBUG3) cconsole.log(new Set([...dset0, ...dset1, ...dset2]).size)
  if (ndoms < 5) {
    this.use_autocomplete = true
    if (DEBUG3) cconsole.warn("use autocomplete! get TLDs from context")
  }
  ...
  if (this.bing_alts.length) {
    for (let alti = 0; alti < this.bing_alts.length || 0; alti++) {
      ...
      // add synonyms
      for (let t = 0; t < 10; t++) {
        if (DEBUG3) cconsole.log(row.key, row.tlds)
        if (row.tlds) {
          if (row.tlds[0] && row.tlds[0][t]) {
            let tld = row.tlds[0][t]
   ...
   */
  if (DEBUG1) {
    cconsole.info("dset0", [...dset0])
    cconsole.info("dset1", [...dset1])
    cconsole.info("dset2", [...dset2])
    cconsole.info("dset3", [...dset3])
    cconsole.info("dset4", [...dset4])
    cconsole.info("dset5", [...dset5])
  }
