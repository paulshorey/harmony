...
import { makeSQLString } from "api/lib/pgdb.js"
...
export const crawl_stopwords_get = function (key) {
  return new Promise(async (resolve) => {
    ...
    pool
      ...
      .catch((err) => {
        if (DEBUG2 || DEBUG1) ccconsole.error(err)
        resolve(null)
      })
  })
}
...
export const data_word_delete = function (key) {
  return new Promise(async (resolve) => {
    ...
    pool
      .query(queryString)
      .then(() => {
        if (DEBUG1) cconsole.warn("deleted ", key)
        resolve(true)
      })
      .catch((err) => {
        if (DEBUG1) cconsole.error(`could not delete "${key}"`, err)
        resolve(null)
      })
  })
}
...
export const data_key_str_unique = async function (row) {
  ...
  if (DEBUG2) cconsole.warn(`data_key_str_unique key "${row.key}" vs key_cleaned "${key_cleaned}"`)
  if (key_cleaned === row.key.toLowerCase()) {
    // no difference - continue
    if (DEBUG2) cconsole.warn("no difference - continue")
  ...
  let row_old = await data_word_get(
    row.key,
  ...
  if (!row_cleaned) {
    ...
    if (DEBUG2) cconsole.warn("no entry for cleaned version: continue", row.key)
    // row = row_meta(row) // fix meta (str, proper, acronym, key)
    return row
  } else if (row.list_count >= row_cleaned.list_count) {
    // dirty key is better: remove cleaned
    if (DEBUG2) cconsole.warn("dirty key is better: remove cleaned", key_cleaned)
    await data_word_delete(key_cleaned)
    // row = row_meta(row) // fix meta (str, proper, acronym, key)
    return row
  } else {
    // cleaned key is better: remove this one (with punctuation)
    if (DEBUG2) cconsole.warn("cleaned key is better: remove this one (with punctuation)", row.key)
...
export const data_word_get = function (str, columns = "", limit = 1000) {
  ...
  return new Promise(async (resolve) => {
    ...
    pool
      ...
      .catch((err) => {
        DEBUG2 || DEBUG1 ? ccconsole.error(err) : null
        resolve(null)
      })
  })
}
...
export const data_word_get_parsed = function (key, columns, limit) {
  if (DEBUG2) cconsole.log("data_word_get_parsed", key)
  return new Promise(async (resolve) => {
...
//       }
//       if (DEBUG2) cconsole.log(queryString)
//       await pool.query(queryString)
//       resolve(true)
//     } catch (e) {
//       DEBUG2 || DEBUG1 ? ccconsole.error(e) : null
//       resolve(false)
//     }
//   })
// }
...
export const data_word_put = async function (row, REBUILD = false) {
  ...
  if (DEBUG2) {
    cconsole.warn("data_word_put", JSON.stringify(row).substr(0, 100))
  } else if (DEBUG1) {
    cconsole.warn("data_word_put", row[primaryKey])
  }
  // required parameters
  if (!row[primaryKey]) {
    return
  ...
  try {
    ...
    // failed
    DEBUG2 || DEBUG1 ? ccconsole.error(e) : null
    cconsole.warn("pg pool failed", e)
...
export const data_word_sentiment_of_synonym = function (key, synonym, sentiment) {
  if (DEBUG1) cconsole.warn("data_word_sentiment_of_synonym", key, synonym, sentiment)
  return new Promise(async (resolve) => {
    // validate
...
export const data_word_remove_words = function ({ key, words }) {
  if (DEBUG1) cconsole.warn("data_word_remove_words", { key, words })
  return new Promise(async (resolve) => {
    // validate
    ...
    if (DEBUG1) cconsole.warn("deleted", row)
    resolve(true)
  })
}
...
export const data_word_add_poswords = function ({ key, pos, poswords: new_words }) {
  let DEBUG1 = false
  if (DEBUG1) cconsole.warn("data_word_add_poswords", { key, pos, new_words })
  return new Promise(async (resolve) => {
    // validate
    if (!key || !pos || !Array.isArray(new_words)) {
      if (DEBUG1) cconsole.error("!key || !pos || !Array.isArray(new_words), exit")
      resolve(false)
    }
    // get row
    let row = await data_word_get_parsed(key)
    if (!row) {
      if (DEBUG1) cconsole.error("!row, exit")
      resolve(false)
    }
    ...
    for (let word of new_words) {
      if (word) {
        ...
        try {
          ...
          if (DEBUG1) cconsole.success(`found row for word="${word}"`)
        } catch (e) {
     ...
     * Log
     */
    if (DEBUG1) cconsole.success(row.key)
    if (DEBUG1) cconsole.success(row.pos1)
    for (let key in row.dict) {
      if (new_words.includes(key)) {
        if (DEBUG1) cconsole.success(key, row.dict[key])
      }
    }
    ...
    if (DEBUG1) cconsole.warn("final row.dict[word] is ...", row.dict && row.dict[(new_words || [])[0]])
    resolve(true)
  })
}
