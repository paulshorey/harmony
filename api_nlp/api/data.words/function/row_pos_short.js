...
export default function (row = {}, options = {}) {
  ...
  for (let pos in row.pos_short) {
    if (DEBUG1) cconsole.info("pos", pos)
    if (DEBUG1) cconsole.info("row.pos_short[pos]", row.pos_short[pos || ""])
  ...
  for (let pos in row.pos_short) {
    ...
    for (let iword in row.pos_short[pos]) {
      ...
      if (DEBUG1) cconsole.log(word, [list_count, length, ratings[word]])
    ...
    // if (DEBUG2) cconsole.log('before sort_strings_by_rating_and_position()', typeof row.pos_short[pos], typeof ratings)
    // row.pos_short[pos] = sort_strings_by_rating_and_position(row.pos_short[pos], ratings, 1)
    // if (DEBUG2) cconsole.log('after sort_strings_by_rating_and_position()')
  }
