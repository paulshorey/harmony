...
import row_model from "api/data.words/function/row_model.js"
import row_meta from "api/data.words/function/row_meta.js"
import row_fw from "api/data.words/function/row_fw.js"
import row_tlds from "api/data.words/promise/row_tlds.js"
import row_poss from "api/data.words/promise/row_poss.js"
import row_pos_short from "api/data.words/function/row_pos_short.js"
import row_food from "api/data.words/function/row_food.js"
...
const thisModule = async function (str, options = {}) {
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      if (DEBUG1 && REBUILD) {
        cconsole.info("str_row", str, options)
      }
      /*
       * initial values
       */
      ...
      if (dbrow) {
         ...
         * FOUND
         */
        if (DEBUG1) cconsole.success(`found row "${row.key}"`)
        row = dbrow
      } else {
        /*
         * NOT FOUND
         */
        if (DEBUG1) cconsole.warn(`!row "${row.key}"`)
        if (options.model === false) {
          /*
           * Return nothing if specified, though by default would continue with empty row
           */
          if (DEBUG1) cconsole.log(`resolve() empty object, because !row and options.model===false "${row.key}"`)
          resolve({})
          return
        } else {
          /*
           ...
           */
          if (DEBUG1) cconsole.log(`resolve() default row for "${row.key}"`)
          row = row_model(row)
          row_meta(row)
          resolve(row)
          return
       ...
       */
      if (REBUILD) {
        if (DEBUG2 && !options.synonyms) cconsole.warn(`rebuild row "${key}"`)
        if (DEBUG1 && options.synonyms) cconsole.success("options.synonyms = ", options.synonyms)
        // time
        ...
        if (options.synonyms) {
          ...
          if (DEBUG1) cconsole.success("added synonyms", add_dict)
        }
        // time
         ...
         * Compile data object
         */
        if (DEBUG2) cconsole.warn("rebuilding row data object... (tlds, meta, pos_short, poss)")
      ...
      if (REBUILD_TLDS) {
        ...
        if (DEBUG2) cconsole.warn("rebuilding row data object... (tlds only)")
        row = await row_tlds(row)
        if (DEBUG2) cconsole.warn("after row_tlds")
        if (DEBUG_TIME) debug_time("rebuilt tlds " + str)
      }
      ...
      if (REBUILD_LIST_COUNT) {
        ...
        if (DEBUG2) cconsole.warn("rebuilding row data object... (list_count/meta/stopwords only)")
        // stopword ? name ? brand
        row_meta(row)
        if (DEBUG2) cconsole.warn("after row_tlds")
       ...
       *
       */
      if (DEBUG2) cconsole.log("resolve() " + typeof row)
      resolve(row)
    } catch (e) {
      cconsole.error("str_row() error!", e)
      resolve()
      return
    }
  })
