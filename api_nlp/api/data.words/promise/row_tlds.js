...
import categories from "data/domains/categories"
import all_preferences from "data/domains/all_preferences.js"
import all from "data/domains/all"
import { sort_strings_by_rating_and_position } from "@twodashes/universal/esm/sort_strings"
import { arr_subtract, arr_remove_item } from "@twodashes/universal/esm/arrays"
import row_model from "../function/row_model"
import fw_words from "data/words/fw/words"
import fw_tlds from "data/words/fw/tlds"
import fw2pos from "data/words/fw/fw2pos"
import fw from "data/words/fw/fw"
...
export default function (row) {
  return new Promise(async (resolve) => {
    ...
    if (DEBUG1) cconsole.warn(row.key, "from DB", row_tlds)
    ...
    let occured6 = {}
    let syns_checked = 0
    if (DEBUG2) cconsole.log(`get key_tlds() for synonyms of key="${row.key}" row_tlds=`, row_tlds)
    ...
    for (let syn of [...row.poss[row.pos1].slice(0, 7)]) {
      ...
      // ILIKE matches in DB
      if (DEBUG2) cconsole.log(`get key_tlds() for syn="${syn}" of key="${row.key}"`)
      // make list from synonym
      let tlds = await key_tlds(syn)
      if (DEBUG2) cconsole.log(tlds)
      ...
      for (let arr_i in arrs) {
        ...
        for (let tld of arr) {
          ...
          if (dict[tld]) {
            ...
            // log
            if (DEBUG2) cconsole.log("." + tld + " => category => ", arr)
          }
        }
      }
    }
    if (DEBUG1) cconsole.log("after categories.reverse")
    ...
    if (DEBUG1) cconsole.log("after abbreviations")

    /*
     * Combine, unique
     */
    if (DEBUG1) {
      cconsole.warn(row.key + " syns tlds", row_tlds)
    }
    // time
    ...
    if (DEBUG1) cconsole.log("after manual tlds0")
    ...
    if (DEBUG1) cconsole.log("after unshift derivations")
    ...
    if (DEBUG1) cconsole.log("after sort")
    ...
    if (DEBUG1) cconsole.warn(row.key, "final", row.tlds)
    resolve(row)
  })
}
