 ...
 * Dependencies
 */
import all_domains from "data/domains/all.js"
import { data_domain_syns1, data_domain_syns2, data_domain_syns3 } from "api/data.domains/pgdb.js"
...
export default async function(key) {
  ...
  if (DEBUG1) cconsole.log(`syns 1 ILIKE "${key}" = `, syns1)

  syns2 = (await data_domain_syns2(key)) || []
  syns2 = syns2.map((d) => d.key)
  if (DEBUG1) cconsole.log(`syns 2 ILIKE "${key}" = `, syns2)

  // let syns3 = (await data_domain_syns3(key)) || []
  // syns3 = syns3.map((d) => d.key)
  // if (DEBUG1) cconsole.log(`syns 3 ILIKE "${key}" = `, syns3)
