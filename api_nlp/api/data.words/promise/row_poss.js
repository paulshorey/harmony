import str_row from "./str_row.js"
import tlds from "data/domains/all.js"
import names from "data/words/names.js"
...
export default async function (row, options) {
  row.poss = { [row.pos1]: [] }
  ...
  if (!tlds[row.key]) {
    ...
    if (row2 && row2.list_count && row2.pos1 === row.pos1 && row2.pos1 === row.pos1) {
       ...
       * debug
       */
      if (DEBUG2) cconsole.log("row2 found", row2.key, row2.pos1)
      /*
       * combine counts
       */
      ...
      if (DEBUG2) cconsole.log("row2 NOT found", row.key, row.singular, row.plural, row.root)
    }
  }
   ...
   *
   */
  if (DEBUG2) cconsole.log("rebuild row.poss")
   ...
   *
   */
  if (DEBUG2) cconsole.log("rebuild row.poss fix before/after")
  // before -
   ...
   * Fix
   */
  if (DEBUG2) cconsole.log("rebuild row.poss fix pos_short")
  ...
  // let row_original = await data_word_get_parsed(row.key)
  ...
  // cconsole.warn('adding row.key to row.dict', row.key)
   ...
   *
   */
  if (DEBUG2) cconsole.log("rebuild row.poss if proper/digit")
  ...
  if (DEBUG2) cconsole.log("rebuild row.poss return " + typeof row)
  return row
}
