...
const is_auth_ip_current = function (ip) {
  ...
  if (!expires || expires < Date.now()) {
    if (DEBUG1) cconsole.info("is_auth_ip_current() !expires || expires < Date.now()")
    return false
  }
  if (DEBUG1) cconsole.log("is_auth_ip_current OK")
  return true
}
...
export default async function (req, { allow_trusted_bypass = true, BYPASS_AUTH = false } = {}) {
  ...
  if (!user.expires) {
    if (BYPASS_AUTH || (allow_trusted_bypass && is_auth_ip_current(req.client_ip))) {
       ...
       *
       */
      if (DEBUG1) cconsole.info("trusted req.client_ip", req.client_ip)
    } else {
      /*
       *
       ...
       *
       */
      if (DEBUG1) cconsole.info("NOT trusted client ip, validate captcha2", req.client_ip)
      /*
      ...
      if (!input_recaptcha2_token && !input_recaptcha3_token) {
        ...
        if (captcha_auth.data.success) {
           ...
           * captcha success
           */
          if (DEBUG1) ccconsole.log("captcha_auth.data.success", req.client_ip)
