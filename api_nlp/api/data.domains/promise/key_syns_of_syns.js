...
import { data_word_get_parsed, data_word_get } from "api/data.words/pgdb.js"
import { data_domain_get_parsed } from "api/data.domains/pgdb.js"
...
export default function (key) {
  return new Promise(async (resolve) => {
    let syns_of_syns = {}
    if (DEBUG2) cconsole.log(`key_syns_of_syns key="${key}"`)
    try {
      /*
       * Get row from DB
       */
      ...
      if (!row || !row.syns1) {
        if (DEBUG1) cconsole.warn(`!row || !row.syns1 for key="${key}"`)
        resolve({})
        return
      }
      if (DEBUG2) cconsole.log(`row="${row.key}" syns1 contains "${row.syns1.length}" words`)
      ...
      if (row.syns1 && row.syns1[0]) {
        ...
        for (let syn1 of row.syns1) {
          let syns_of_syn1 = []
          let row1 = await data_word_get_parsed(
            syn1,
          ...
          if (row1 && row1.list_count) {
            if (DEBUG1) cconsole.success(`found row1 key="${row1.key}" list_count="${row1.list_count}"`)
            ...
            if (DEBUG2) cconsole.log("row1 list1", list1)
            /*
             * Syns *2
             */
            // use list list only if word is positive sentiment - if negative, use all available words
            if (DEBUG2) cconsole.log(`syn1="${syn1}" list contains "${list1.length}" words`)
            // if (row1["x" + row1.pos1]) {
            ...
            for (let syn2 of [row1.key, row1.root, row1.singular, row1.plural, ...list1].filter((str) => !!str)) {
              let row2 = await data_word_get_parsed(
                syn2,
              ...
              if (row2 && row2.list_count) {
                ...
                if (DEBUG2) cconsole.log("row2 list2", list2)
                /*
                ...
                //   let row3 = await data_word_get_parsed(syn3, "key,root,pos1,singular,plural,list_count")
                //   if (row3 && row3.list_count) {
                //     if (DEBUG1) cconsole.info(`row3 key="${row3.key}", list_count="${row3.list_count}"`)
                //     // push self
                ...
                //     //   let row = await data_word_get(row3.singular.toLowerCase(), "key,list_count")
                //     //   if (row && row.list_count >= 10) {
                //     //     syns_of_syn1.push("__" + row.key)
                //     //   }
                //     // }
                //     // if (row3.plural && row3.plural !== row3.key) {
                //     //   let row = await data_word_get(row3.plural.toLowerCase(), "key,list_count")
                //     //   if (row && row.list_count >= 10) {
                //     //     syns_of_syn1.push("__" + row.key)
                //     //   }
                //     // }
                //     if (DEBUG1) cconsole.info(`row3 key done`)
                //   }
                // }
              }
            }
          } else {
            if (DEBUG1) cconsole.warn(`not found row for syn1="${syn1}"`)
          }
          /*
