 ...
 * IMPORTS
 */
import { data_domains_get_all } from "api/data.domains/pgdb.js"

import domain_key from "api/data.domains/promise/key" // get one domain (from key)
import domain_row_update from "api/data.domains/promise/row_update.js" // get row (from row), but update synonyms first
import domain_syns_of_syns from "api/data.domains/promise/key_syns_of_syns.js" // get dictionary of syns1:syns2
...
export default function (expressApp, http_response) {
   ...
   * @returns: row
   */
  expressApp.get("/api/data/domain/:key", async function (req, res) { global.res = res
    try {
      validate_req(req)
    ...
    if (process.env._system_name !== "OSX") {
      if (DEBUG1) ccconsole.info(`GET "/api/data/domain/${req.params.key}"`, { host: global["hostname"] })
      /*
       * Too many requests ?!?
       */
      requests_count++
   ...
   */
  expressApp.get("/api/data/domains", async function (req, res) { global.res = res
    try {
      validate_req(req)
   ...
   */
  expressApp.get("/api/data/domain_syns_of_syns/:key", async function (req, res) { global.res = res
    try {
      validate_req(req)
   ...
   */
  expressApp.put("/api/data/domain/:key", async function (req, res) { global.res = res
    try {
      validate_req(req)
