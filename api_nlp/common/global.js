...
const cloudConsoleLog = function () {
  dnaConsole.log(arguments, { level: "log" })
}
const cloudConsoleInfo = function () {
  dnaConsole.log(arguments, { level: "info" })
}
const cloudConsoleWarn = function () {
  dnaConsole.log(arguments, { level: "warn" })
}
const cloudConsoleError = function () {
  // global['airbrake'].notify(arguments)
  dnaConsole.log(arguments, { level: "error" })
}

//
// log to console
//
import cconsoleSetup from "colorful-console-logger/cconsoleInit"
// debug
global["cconsole"] = cconsoleSetup({
  useTrace: true,
  useColor: true,
  logToCloud: {
    log: cloudConsoleLog,
...
// cloud-debug
global["ccconsole"] = cconsoleSetup({
  useTrace: true,
  useColor: true,
  logToCloud: {
    log: cloudConsoleLog,
