...
import { data_get_words, data_word_put } from "api/data.words/pgdb.js"
...
const okgo = async function () {
  ...
  for (let row of rows) {
    try {
       ...
       * Debug
       */
      if (DEBUG1) cconsole.info(`editing row`, row)
      ...
      for (let key of row.abbreviation.split(",")) {
         ...
         * Get/Edit row
         */
        let row2 = await str_row(key, { REBUILD: true })
        if (!row2) {
          if (DEBUG1) cconsole.error("!row2")
          continue
        }
         ...
         * LOG
         */
        if (DEBUG1) cconsole.log({ key: row2.key, abbreviation: row2.abbreviation, list_count: row2.list_count })
      }
    } catch (e) {
