...
anonFunction(async () => {
  ...
  for (let tuple of abbreviations) {
    if (DEBUG1) cconsole.log("tuple", tuple)
    /*
    ...
    if (DEBUG1) cconsole.log("abbrs", abbrs)
    ...
    for (let word of words) {
      ...
      cconsole.success("got row from key_row()", row ? row.key : row)
      words_rows.push(row)
      // fix
      ...
      // save
      if (DEBUG1) cconsole.info(`adding abbreviations "${row.abbreviation}" to word "${row.key}"`)
    ...
    for (let abbr of abbrs) {
      ...
      // save row
      if (DEBUG1) {
        cconsole.info(`adding words "${words_to_add_to_abbrs.map((row) => row.key)}" to abbreviation "${abbr}"`)
      }
      await data_word_put(row)
    }
  }
