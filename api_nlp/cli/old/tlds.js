...
;(async function () {
  ...
  for (let row of rows) {
    ...
    if (rowEdited) {
      if (DEBUG2) cconsole.success(`updated ${rowEdited.key}`)
      if (DEBUG2) cconsole.log(rowEdited)
    } else {
      if (DEBUG2) cconsole.warn(`could not update row`)
    }
    ...
    if (DEBUG2) cconsole.log("data_domain_put success")

    /*
     * LOG
     */
    if (row_syns2 && rowEdited && row_syns2.length !== rowEdited.syns2.length) {
      cconsole.warn(row.key + ` fixed: before=(${row_syns2.length}) after=(${rowEdited.syns2.length})`)
    } else {
      cconsole.info(row.key + " ok")
    }
  }
