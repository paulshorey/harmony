...
module.exports = function ($) {
  ...
  function filterWord_minLengthThis(word) {
    ...
    if (THIS.toDelete.includes(word) && this.type !== "gerunds" && this.type !== "keys" && this.type !== "roots" && this.type !== "plurals") {
      return false;
    }
    // default ok
    return true;
  ...
  function wd_sense({key, pos, element}) {
    ...
    element.children().each((i, liElement) => {
      ...
      for (let txt of words) {
        ...
        if (txt.indexOf("Synonyms:") !== -1 || txt.indexOf("to:") !== -1) { // "to:" // synonyms
          type = "synonyms";
        } else if (txt.indexOf("Topic:") !== -1 || txt.indexOf("of:") !== -1) { // "of:" // types
          type = "types";
        } else if (txt.indexOf("eriv") !== -1 || txt.indexOf("from:") !== -1) { // "Derived from:"
          type = "derivations";
        } else if (txt.indexOf(":") !== -1) { // Other ":" will be ignored
          type = "";
  ...
  const output = (function () {
    ...
    var replaced = $("body").html().replace(/(<b>)?[a-z]+(<\/b>)?\.\.\./gi, '');
        ...
        switch (tag) {
          ...
          case("ul"):
            if (senses_key) {
              ...
              if (THIS.senses[pos].derivations.length || THIS.senses[pos].definitions.length || THIS.senses[pos].synonyms.length || THIS.senses[pos].types.length) {
    ...
    if ($('#sw').length) {
      ...
      if (!THIS.sense) {
        ...
        var verbs = [...THIS.derivations["3rd-person"], ...THIS.derivations["past participle"], ...THIS.derivations["present participle"], ...THIS.derivations["present tense"], ...THIS.derivations["past tense"], ...THIS.derivations["future tense"]];
        if (pronouns.length) {
          THIS.sense = "pronouns";
      ...
      $('#sw .t .r').each(function (i, el) {
        ...
        // DERIVATIVES
        else if (txt.indexOf("from:") !== -1 || txt.indexOf("Derivative:") !== -1 || txt.indexOf("Related to:") !== -1 || txt.indexOf("Comparative:") !== -1 || txt.indexOf("Superlative:") !== -1) {
          $el.find('p a').each(function (i, el) {
            let $el = $(el);
            let txt = $el.text().trim();
            if (minLen(txt) && txt.length < 22) {
    ...
    $('#r .r h4:first-child').each(function (bE, section) {
      ...
      // remove result
      if (!appearsIn && [...THIS.derivations.sense, ...THIS.derivations['past tense'], ...THIS.derivations.plural].length > 10) {
        $section.remove();
      } else {
     ...
     */
    // fix NOUNS
    if (THIS.senses.nouns.derivations.length || THIS.senses.nouns.synonyms.length || THIS.senses.nouns.abbreviations.length) {
    ...
    for (const pos in THIS.senses) {
      ...
      const propers = text.match(/"([A-Z]{1}[a-z-]{2,} ?[A-Z]{0,1}[a-z- ]{0,} ?[A-Z]{0,1}[a-z- ]{0,} ?[A-Z]{0,1}[a-z- ]{0,} ?[A-Z]{0,1}[a-z- ]{0,})/g);
            ...
            if (!shortestRoot || word.length < shortestRoot.length) {
              ...
              if (word.substr(0, until).toLowerCase() === key.substr(0, until).toLowerCase()) {
                ...
                if (word.indexOf(' ') === -1) {
                  ...
                  if (word.substr(-1).toLowerCase() !== 'a' && word.substr(-1).toLowerCase() !== 'i' && word.substr(-1).toLowerCase() !== 'o' && word.substr(-1).toLowerCase() !== 'u' && word.substr(-1).toLowerCase() !== 'y') {
    ...
    for (const pos in THIS.senses) {
      ...
      let senseJSONList = [...sense.derivations, ...sense.keys, ...sense.plurals, ...sense.definitions, ...sense.propers];
        ...
        if (type === "keys" || type === "propers") {
          ...
          if (THIS.key && type !== "plurals") {
            sense[type] = sense[type].filter(
              filterWord_removeThis.bind({type: type, key: THIS.key.toLowerCase()})
            );
          }
          // remove 2-letter strings
          let pronounAbbreviation = false;
          if (THIS.sense === "pronouns" || THIS.sense === "abbreviations" || (sense.abbreviations || []).length || (sense.pronouns || []).length) {
      ...
      for (const chosen_type in sense) {
        ...
        for (const chosen_word of chosen_words_list) {
          ...
          for (const type in sense) {
            ...
            if (type === "keys" || type === "plurals" || type === "propers" || type === "roots" || type === chosen_type) {
              continue;
            }
    ...
    for (const pos in posWords) {
      ...
      for (let type in sense) {
        ...
        if (type === "keys" || type === "roots" || type === "gerunds" || type === "plurals" || type === "abbreviations") {
    ...
    for (const pos in posWords) {
      ...
      if (pos !== 'etc') {
        ...
        // matches
        if ((properExact && properExact.length >= 1) || (properStart && properStart.length > 1) || (properEnd && properEnd.length > 1)) {
      ...
      if (pos !== 'etc') {
        ...
        const properEnd = text.match(new RegExp(`"[\w -']*?${Proper}"`, "g"));
        // matches
        if ((properExact && properExact.length >= 2) || (properStart && properStart.length > 2) || (properEnd && properEnd.length > 2)) {
          posWords[pos].propers.push(Proper); // start after first character, to account for leading `"`
        }
      }
      ...
      if (pos !== 'etc') {
        ...
        const properEnd = text.match(new RegExp(`"[\w -']*?${Proper}"`, "g"));
        // matches
        if ((properExact && properExact.length >= 3) || (properStart && properStart.length > 3) || (properEnd && properEnd.length > 3)) {
          posWords[pos].propers.push(Proper); // start after first character, to account for leading `"`
        }
      }
