...
import { Row, Col, Switch, Slider, InputNumber, Collapse, List, Typography, Input } from "antd";
...
import { defaultProps } from "@ps/ui/carousel/types";
...
const CustomSlide = ({ value, onChange, ...options }) => (
  <Row>
    <Col span={14}>
      <Slider {...options} value={value} onChange={onChange} />
    </Col>
    <Col span={9} offset={1}>
      <InputNumber
        {...options}
...
const CustomSwitch = ({ value, onChange, ...options }) => <Switch {...options} checked={value} onChange={onChange} />;
const CustomInput = ({ value, onChange, ...options }) => <Input {...options} value={value} onChange={(e) => onChange(e.currentTarget.value)} />;
...
class CustomSlider extends Component {
  ...
  format = (options) =>
    map(options || this.datas, (data, index) => {
      let result = null;
      const { name, component } = data;
      if (typeof component === "object" && Array.isArray(component)) {
        result = (
          <Collapse.Panel header={name} key={index + 1}>
            <List itemLayout="horizontal" dataSource={component} renderItem={(item) => <List.Item>{this.createComponent(item)}</List.Item>} />
          </Collapse.Panel>
        );
      } else {
  ...
  render() {
    ...
    Object.keys(diff)
      .sort()
      .forEach((key) => diffArray.push({ key, value: diff[key] }));
    return (
      <Row>
        <Col span={8}>
          <Collapse accordion>
            <Collapse.Panel header="boxCount" key={0}>
              <Row style={{ width: "100%" }}>
                ...
                <Col offset={1} span={22}>
                  <CustomSlide min={1} max={100} value={boxCount.length} onChange={(value) => this.setState({ boxCount: new Array(value) })} />
                </Col>
              </Row>
            </Collapse.Panel>
            {this.format()}
